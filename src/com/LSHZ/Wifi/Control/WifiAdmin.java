...
//      mTimer.schedule(mTimerTask, 0, 20 * 1000);// 20s  
    ...
    {  
        ...
        for (int i = 0; i < mWifiList.size(); i++) {  
            list.add(new WifiDevice(mWifiList.get(i).BSSID,mWifiList.get(i).SSID));  
        }  
        return list;
    }  
    
    public List<WifiDevice> GetWifiDeviceList(String sign) 
    {  
        List<WifiDevice> list=new ArrayList<WifiDevice>();  
        for (int i = 0; i < mWifiList.size(); i++) {  
            if(mWifiList.get(i).SSID.contains(sign))
            	list.add(new WifiDevice(mWifiList.get(i).BSSID,mWifiList.get(i).SSID));  
        }  
        return list;
    }  
